...
GOAL_ZH = (
    "理解用户输入信息，从已知信息的意图定义中选择一个和用户输入最匹配的意图，"
    "并按要求输出意图信息."
)
...
RETRY_GOAL_EN = (
    ...
    "information in the recent messages, and return the supplemented intent "
    "information."
)

RETRY_GOAL_ZH = (
    "阅读下面用户提供的最近消息内容，并把当前用户输入信息"
    "提取补充到最近消息中的意图信息里，并返回补充后的意图信息。"
)

CONSTRAINTS_EN = [
    "Strictly define the output based on the given intent. Do not generate the intent "
    "and slot attributes by yourself. If the intent does not define a slot, the output "
    "should not include the slot.",
    ...
    "If there is no 'Slots' definition in the selected intent definition, make sure "
    "the output slot content is empty and do not create any slot information yourself.",
    "If there is a 'Slots' definition in the selected intent definition, extract the "
    "value of the specific slot attribute from the user input. If the value of the slot"
    " attribute cannot be obtained, ensure that the slot attribute value output is "
    "empty.",
    "When extracting the slot value, please be careful to only obtain the valid value "
    "part. Do not fill in auxiliary descriptions, attributes, prompts or description "
    "information, and do not appear content like 'not provided by the user'.",
    "When the complete slot attribute value defined by the intent cannot be collected,"
    " a prompt will be proactively sent to the user to remind the user to supplement "
    "the missing slot data.",
]
CONSTRAINTS_ZH = [
...
RETRY_CONSTRAINTS_EN = [
    "Read the recent message dialogue information provided by the user, extract and "
    "supplement the information input by the current user into the historical dialogue"
    " intent, and output it in the original intent format without losing attributes.",
    "If the user does not explicitly request modification, do not modify the existing"
    " intent and slot value in the intent in the recent message, only add new content.",
    ...
    "Extract the value of the slot in the intent definition from user input and recent"
    " messages. If the target information corresponding to the slot cannot be obtained,"
    " the slot value output is empty.",
    "Make sure that all slot attributes of the slot defined by the intention are output"
    " regardless of whether the value is obtained, and the slot name and null value are"
    " output if the value is not found.",
    "If the slot value still cannot be completely filled, new questions will continue "
    "to be generated to ask the user, prompting the user to supplement the missing slot"
    " data.",
]
