...
class PluginHub:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Import):
                for name in node.names:
                    if name.name in self.disallowed_imports:
                        raise ValueError(f"Plugin contains disallowed import: {name.name}")
            ...
            # Check for from ... import statements
                ...
                for name in node.names:
                    ...
                    if combined in self.disallowed_imports or name.name in self.disallowed_imports:
                        raise ValueError(f"Plugin contains disallowed import: {combined}")
            
            # Check for calls to dangerous functions
            elif isinstance(node, ast.Call):
                if isinstance(node.func, ast.Name):
                    if node.func.id in {"eval", "exec", "compile"}:
                        raise ValueError(f"Plugin contains potentially dangerous function call: {node.func.id}")
                elif isinstance(node.func, ast.Attribute):
                    if isinstance(node.func.value, ast.Name):
                        if node.func.value.id == "os" and node.func.attr in {"system", "popen", "spawn", "exec"}:
                            raise ValueError(f"Plugin contains potentially dangerous function call: os.{node.func.attr}")
    ...
    async def upload_my_plugin(self, doc_file: UploadFile, user: Any = Default_User):
        ...
        if safe_filename.lower().endswith('.zip'):
            import zipfile
            extract_dir = os.path.join(self.plugin_dir, f"temp_extract_{os.path.splitext(safe_filename)[0]}")
            try:
                with zipfile.ZipFile(file_path, 'r') as zip_ref:
                    ...
                    for zip_info in zip_ref.infolist():
                        if zip_info.file_size > 10 * 1024 * 1024:  # 10MB limit for any single file
