...
export default function ArgumentsModal({ space, argumentsShow, setArgumentsShow }: IProps) {
      ...
      <Row gutter={24}>
        ...
        <Col span={12}>
          <Form.Item<IArguments>
            ...
            rules={[{ required: true }]}
            label={i18n.t('recall_score')}
            name={['embedding', 'recall_score']}
          >
            <Input className='mb-5  h-12' placeholder={i18n.t('Please_Input')} />
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item<IArguments>
        ...
        <Col span={12}>
          <Form.Item<IArguments>
            ...
            rules={[{ required: true }]}
            label={i18n.t('chunk_overlap')}
            name={['embedding', 'chunk_overlap']}
          >
            <Input className='mb-5  h-12' placeholder={i18n.t('Please_input_the_description')} />
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item<IArguments>
            ...
            rules={[{ required: true }]}
            label={i18n.t('retrieve_mode')}
            name={['embedding', 'retrieve_mode']}
          >
            <Select className='mb-5  h-12' placeholder={i18n.t('Please_input_the_description')}>
  ...
  const renderPromptForm = () => {
    return (
      <>
        <Form.Item<IArguments> tooltip={i18n.t(`A_contextual_parameter`)} label={i18n.t('scene')} name={['prompt', 'scene']}>
          <TextArea rows={4} className='mb-2' />
        </Form.Item>
        <Form.Item<IArguments> tooltip={i18n.t(`structure_or_format`)} label={i18n.t('template')} name={['prompt', 'template']}>
          <TextArea rows={7} className='mb-2' />
        </Form.Item>
        <Form.Item<IArguments>
          tooltip={t(`The_maximum_number_of_tokens`)}
          label={i18n.t('max_token')}
  ...
  const renderSummary = () => {
    return (
      <>
        ...
        <Form.Item<IArguments>
          rules={[{ required: true }]}
          label={i18n.t('concurrency_limit')}
