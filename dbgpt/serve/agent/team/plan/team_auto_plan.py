...
class AutoPlanChatManager(ManagerAgent):
    ...
    goal: str = (
        "Advance the task plan generated by the planning agent. If the plan does not pre-allocate an agent, it needs to be coordinated with the appropriate agent to complete."
    )
