...
class DashboardDataLoader:
    ...
    def get_chart_values_by_data(self, field_names, datas, chart_sql: str):
        ...
        data_map.update(
            {f"{field_name}": [row[index] for row in datas] for index, field_name in enumerate(field_names)})
        # to Check Whether there are data in it
        if len(datas) != 0:
            # find the first string column
            str_index = next((index for index, value in enumerate(datas[0]) if isinstance(value, str)), 1)
            ...
            return field_names, [ValueItem(name=f'{field_name}', type=f"{field_name}", value='0') for index, field_name
