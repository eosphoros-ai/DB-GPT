...
class GptsPlansEntity(Base):
    ...
    status = Column(String(255), nullable=True, comment="subtask status")
...
class GptsPlansDao(BaseDao[GptsPlansEntity]):
    ...
    def batch_save(self, plans: list[GptsPlansEntity]):
    ...
    def complete_task(self, task_id: str, result: str):
        ...
        gpts_plans = gpts_plans.filter(GptsPlansEntity.id == task_id)
        gpts_plans.update({
            GptsPlansEntity.status: Status.COMPLETED.value,
            GptsPlansEntity.result: result
        }, synchronize_session='fetch')
        session.commit()
        session.close()

    def update_task(self, task_id: int, status: str, retry_times: int, agent: str = None):
        session = self.get_session()
        gpts_plans = session.query(GptsPlansEntity)
        gpts_plans = gpts_plans.filter(
            GptsPlansEntity.id == task_id)
        gpts_plans.update({
            GptsPlansEntity.status: status,
